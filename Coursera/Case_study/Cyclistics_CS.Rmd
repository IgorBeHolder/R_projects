...
df %>% 
  ...
  ggplot() + 
  geom_col(aes(y = ride_count, x = member_casual, fill = rideable_type ),
           show.legend = TRUE) + #, position=position_stack(reverse = FALSE)
  geom_label(aes(y = ride_count, x = member_casual,  
                 label = format(ride_count,big.mark=",") ),
             vjust = 0, position = position_stack(vjust = 0.6) ) + 
...
Casual riders prefer classic bikes while members choose electric bikes.\
Cyclistics members use services much more often than casual riders.
...
Average duration of casual riders is significantly higher.
