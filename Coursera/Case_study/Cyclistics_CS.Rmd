---
title: "Coursera Capstone project for Google Data Analytics Professional Certificate. Cyclistic bike-share. R Notebook"
author: "Igr Sorochan"
date: "`r Sys.Date()`"
output:
  html_document:
...
customers. Each trip is anonymized.\
...
The data has to be processed to remove:

-    trips that are taken by staff as they service and inspect the
    system

-   any trips that were below 60 seconds in length (potentially false
    starts or users trying to re-dock a bike to ensure it was secure).

Let's take a look on "strange" observations\
...
The surge at mark 25h(1500 min) might be a service notation, e.g. bikes
that had been left out of parking stations, stolen bikes or defected.

Finally, we'll consider trip a **valid** if it meets following
conditions:

1.  `rideable_type != "docked_bike"` to exclude service observations.

2.  `trip_duration` \> 1 minute (false starts and re-docking).

3.  `trip_duration` \< 1499 minutes ( mean + 8 std dev)
...
Checking for integrity `trip_df`:
...
We've managed to restore station's names at 324.892 observations.\
...
-   the upper limit to `mean + 5sigma` = 141 minutes
...
It removes only 24.651 rows (0.4 % of all trips). So our conclusions
...
Casual riders prefer electric bikes (+42.8% over classic bikes) \
while members more often choose classic bikes.\
Members use Cyclistics's services much more often than casual riders (+
61 %).

|                | casual riders       | members            | total     |
|----------------|---------------------|--------------------|-----------|
| classic bikes  | 890.944             | 1.779.640          | 2.670.584 |
| electric bikes | 1.272.693 (+42.8 %) | 1.703.416 (-4.3 %) | 2.976.109 |
| total bikes    | 2.163.137           | 3.483.056          | 5.646.693 |
...
raw_df %>% 
  ...
  # filter(trip_duration > as.duration(93596/60)) %>% 
  arrange(desc(trip_duration))
```
...
| Sample        | The mode | Number of trips         |
|---------------|----------|-------------------------|
| Member riders | Thursday | 9806 (8094 at Saturday) |
| Casual riders | Saturday | 7714                    |
| Population    | Saturday | 15808                   |

: The mode of day of week

The mode of day of week in population is Saturday.
